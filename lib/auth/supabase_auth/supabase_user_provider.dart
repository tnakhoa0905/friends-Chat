...
class ChatAppSupabaseUser extends BaseAuthUser {
  ChatAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> chatAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ChatAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
